...
class MinMaxHeap{
	...
	void PrintMaxItems(){ // 印出  bottom leftbottom 的編碼及資料 
		for( int i = 0; i < maxitems.size() ; i++)	cout << "[" << i+1 <<"]"<<maxitems[i].num << ", " << maxitems[i].school << maxitems[i].department << ", " <<
	...
	*/
	...
	void Delete(int index){ 
		...
		while( BiggestGrandchild(index) != -1 && minmaxheap[index].graduate < minmaxheap[BiggestGrandchild(index)].graduate ) {
	...
	void GetMax( int k){
		for( ;k > 0 ; k--){
			if( minmaxheap[1].graduate >= minmaxheap[2].graduate) maxitems.push_back(minmaxheap[1]),Delete(1);
			else maxitems.push_back(minmaxheap[2]),Delete(2);
		}
	}
};
...
int main(){
	...
	while( fileName != "0" ){
		...
		cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl;
		cout << "@@@         Mission 2: Min-max Heap Construction         @@@" << endl;
		cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl;
		cout << "enter k" << endl;
		int k =0;
		cin >> k;//輸入使用者要幾個MAx值 
